...
- If data/enroll/<name> already contains aligned crops, those are loaded,
  embedded again, and INCLUDED in the template.
- New captures are appended.
...
def main():
    ...
    print("\nEnrollment started.")
    if base_samples:
        print(f"Re-enroll mode: found {len(base_samples)} samples in {person_dir}/")
        ...
        while True:
            ...
            if key == ord("s"):
                ...
                if total < max(3, cfg.samples_needed // 2):
                    status_msg = f"Not enough samples to save (have {total})."
                    continue
                ...
                meta = {
                    ...
                    "samples_total_used": len(all_samples),
